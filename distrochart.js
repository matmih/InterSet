...
*/
    ...
    // index: the ordinal position of the key within the object 
		forText.push(chart.groupObjs[key].values.length);
		});
		
		var texttoWrite = "";
		var wordSp=0; var move=0;
		
		if(forText.length == 3){
			texttoWrite = forText[0]+(new Array(5).join(' '))+forText[1]+(new Array(5).join(' '))+forText[2];
			wordSp=75;
			move=35;
			for(var i=1;i<forText.length;i++)
				if(forText[i]>99)
					wordSp=wordSp-5;
		}
		else if(forText.length==4){
			texttoWrite = forText[0]+'   '+forText[1]+'   '+forText[2]+'   '+forText[3];
			wordSp=50;
			move=22;
			for(var i=1;i<forText.length;i++)
				if(forText[i]>99)
		...
		else if(forText.length==5){
			texttoWrite = forText[0]+'  '+forText[1]+'  '+forText[2]+'  '+forText[3]+' '+forText[4];
			wordSp=35;
			move=20;
			for(var i=1;i<forText.length;i++)
				if(forText[i]>99)
		...
		if(forText[0]<10)
			move=move+10;
		else if(forText[0]>9 && forText[0]<100)
			move=move+5;
		else if(forText[0]>999 && forText[0]<10000)
			move = move-5;
		else if(forText[0]>10000)
			move = move - 15;//not good yet, look at each number individually
	
        // Create the svg
       
		var tmpc= chart.objs.chartDiv.append("svg")
            .attr("class", "chart-area")
            .attr("width", chart.width + (chart.margin.left + chart.margin.right))
            .attr("height", chart.height + (chart.margin.top + chart.margin.bottom));
			
			tmpc.append("svg:text").attr("x", (0+chart.margin.left+move)).attr("y", 20).attr("font-family","sans-serif").attr("font-size","15px").attr("fill","red").attr("align","center")
			.attr("lengthAdjust","spacingAndGlyphs").attr("word-spacing",wordSp).text(texttoWrite);
//
//.attr("textLength",chart.width - (chart.margin.left - chart.margin.right-35))
		   chart.objs.g  = tmpc.append("g")
        ...
        // Create tooltip div
        chart.objs.tooltip = chart.objs.mainDiv.append('div').attr('class', 'tooltipViol').style("position", "absolute").style("text-align","left").style("padding","10px").style("font", "12px sans-serif")
		.style("background", "#e6eeff").style("border", "0px").style("border-radius","1px").style("pointer-events","none").style("opacity", "0.7").style("display", "none");
            ...
            for (cName in chart.groupObjs) {
                ...
                if (vOpts.showViolinPlot !== false) {
                    ...
                    cViolinPlot.objs.left.area = cViolinPlot.objs.left.g.append("path")
                        .attr("class", "area")
                        .style("fill", chart.violinPlots.color(cName)).style("shape-rendering","geometricPrecision").style("opacity", "0.4");
    ...
    chart.renderBoxPlot = function (options) {
        ...
        !function calcAllOutliers() {
            ...
            function calcOutliers(cGroup) {
                ...
                for (idx = 0; idx <= cGroup.values.length; idx++) {
					if(isNaN(cGroup.values[idx]) || typeof(cGroup.values[idx])=="undefined")
						continue;
            ...
            for (cName in chart.groupObjs) {
                ...
                if (cBoxPlot.objs.outliers) {
                    for (pt in cBoxPlot.objs.outliers) {
						...
						if(typeof(cBoxPlot.objs.outliers[pt].point)=="undefined"){
							continue;
						}
						
						if(isNaN(chart.yScale(cBoxPlot.objs.outliers[pt].value))){
							continue;
						}
                ...
                if (cBoxPlot.objs.extremes) {
                    for (pt in cBoxPlot.objs.extremes) {
						if(typeof(cBoxPlot.objs.extremes[pt].point)=="undefined"){
							continue;
						}
						
						if(isNaN(chart.yScale(cBoxPlot.objs.extremes[pt].value))){
							continue;
						}
        ...
        chart.boxPlots.prepareBoxPlot = function () {
            ...
            for (cName in chart.groupObjs) {
                ...
                cBoxPlot.objs.g = chart.groupObjs[cName].g.append("g").attr("class", "box-plot").style("stroke-width", "2");
                ...
                if (bOpts.showMedian) {
                    ...
                    cBoxPlot.objs.median.line = cBoxPlot.objs.g.append("line")
                        .attr("class", "median").style("stroke-width", "2px");
                ...
                if (bOpts.showMean) {
                    ...
                    cBoxPlot.objs.mean.line = cBoxPlot.objs.g.append("line")
                        .attr("class", "mean").style("stroke-width", "2px");
                ...
                if (bOpts.showOutliers) {
                    ...
                    if (cBoxPlot.objs.outliers.length) {
						...
						if(isNaN(chart.yScale(cBoxPlot.objs.outliers[pt].value)) || typeof(cBoxPlot.objs.outliers[pt].value) == "undefined"){
							continue;
						}
                    ...
                    if (cBoxPlot.objs.extremes.length) {
						...
						if(isNaN(chart.yScale(cBoxPlot.objs.extremes[pt].value)) || typeof(cBoxPlot.objs.extremes[pt].value) == "undefined"){
							continue;
						}
    ...
    chart.renderDataPlots = function (options) {
        ...
        chart.dataPlots.preparePlots = function () {
            ...
            if (dOpts.showLines && dOpts.showLines.length > 0) {
                ...
                var cMetric;
				console.log(dOpts.showLines);
                for (var i=0;i<dOpts.showLines.length;i++) {
					var line = dOpts.showLines[i];
					console.log(line);
                    cMetric = line;//dOpts.showLines[line];
